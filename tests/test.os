...
Процедура ПереопределениеТекстаСценария() Экспорт
	...
	КонструкторПрокси = Новый КонструкторПрокси(Новый МойКласс())
		.ДобавитьПубличноеПоле("МоеПоле");
	ТекстСценария = КонструкторПрокси.ТекстСценария();
	ТекстСценария = ТекстСценария + Символы.ПС + "МоеПоле = 100;";

	Прокси = КонструкторПрокси.Построить(ТекстСценария);

	Ожидаем.Что(Прокси.МоеПоле).Равно(100);

КонецПроцедуры

&Тест
Процедура ПереопределилсяВозвращяемыйЭтотОбъект() Экспорт
	...
	Результат = Прокси.ВернутьЭтотОбъект().ДобавленнаяФункция();
